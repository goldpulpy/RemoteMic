...
[📦 Установка](#installation) · [🚀 Быстрый старт](#quick-start) · [🎚 Качество аудио](#audio-quality) · [⌨ CLI](#command-line-options)

[⚡ Автозапуск](#systemd-service) · [🔒 Безопасность](#security-model) · [❓ Устранение неполадок](#troubleshooting) · [📄 Лицензия](#license)
...
При установке в стандартный каталог `~/.local/bin` установщик при необходимости
добавляет его в конфигурацию обнаруженной оболочки Bash, Zsh, Fish или POSIX.
После первой установки перезапустите терминал. Флаг `--no-modify-path` оставляет
конфигурацию оболочки без изменений.
...
Остановите RemoteMic через `Ctrl+C`, затем удалите готовый бинарный файл из
каталога, который установщик использует по умолчанию:

```bash
rm -- "$HOME/.local/bin/remotemic"
```

Если при установке был указан параметр `--install-dir`, удалите `remotemic` из
выбранного каталога. Например, установку с `--install-dir /usr/local/bin` можно
удалить командой `sudo rm -- /usr/local/bin/remotemic`.
...
Если RemoteMic был собран из исходников, удалите клонированный репозиторий или
бинарный файл, который вы копировали вручную.
